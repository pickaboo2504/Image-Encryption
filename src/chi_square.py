...
import hashlib
import numpy as np
from scipy import stats  # We'll use this for precise critical value calculation
...
# def calculate_chi_square(image_array):
#     """Calculate chi-square value for image histogram uniformity test."""
...
#     original_chi = calculate_chi_square(image_array)
#     encrypted_chi = calculate_chi_square(cipherImage)
    
#     print(f"\nChi-square values:")
#     print(f"Original image: {original_chi:.4f}")
#     print(f"Encrypted image: {encrypted_chi:.4f}")
    
#     # Interpretation
#     print("\nInterpretation:")
#     print("A lower chi-square value indicates better uniformity in the histogram.")
#     print("For 256 bins (degrees of freedom = 255), the critical value at 0.05 significance is about 293.")
#     print("Values below this suggest the histogram is sufficiently uniform.")
...
def main():
    ...
    # Get the precise critical value for δ=0.04 and df=255
    significance_level = 0.04
    degrees_of_freedom = 255
    critical_value = stats.chi2.ppf(1 - significance_level, degrees_of_freedom)
    
    print(f"\nChi-square analysis (δ={significance_level}, df={degrees_of_freedom}):")
    print(f"Critical value: {critical_value:.4f}")
    print(f"Original image: {original_chi:.4f}")
    print(f"Encrypted image: {encrypted_chi:.4f}")
    
    print("\nInterpretation:")
    print(f"Values below {critical_value:.4f} indicate sufficient uniformity at {significance_level*100}% significance level.")
    if encrypted_chi < critical_value:
        print("SUCCESS: Encrypted image histogram is sufficiently uniform.")
    else:
        print("WARNING: Encrypted image histogram does not show sufficient uniformity.")
